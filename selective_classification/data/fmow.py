...
class FMoWDataset(WILDSDataset):
    ...
    def __init__(
        ...
        countrycode_to_region = {
            k: v for k, v in zip(country_codes_df["alpha-3"], country_codes_df["region"])
        }
        regions = [
        ...
        if self._split_scheme.startswith("time_after"):
            ...
            self.val_ood_mask = (
                np.asarray(pd.to_datetime(self.metadata["timestamp"]) >= year_minus_3_dt)
                & ~self.test_ood_mask
            )
        ...
        for split in self._split_dict.keys():
            ...
            if self.oracle_training_set and split == "train":
                ...
                subsample_unused_ood_idxs = subsample_idxs(
                    unused_ood_idxs, num=len(idxs) // 2, seed=self.seed + 2
                )
                subsample_train_idxs = subsample_idxs(
                    idxs.copy(), num=len(idxs) // 2, seed=self.seed + 3
                )
        ...
        region_idxs = [
            region_to_region_idx[region] for region in self.metadata["region"].tolist()
        ]
        ...
        for year in range(2002, 2018):
            year_mask = np.asarray(
                ts >= datetime.datetime(year, 1, 1, tzinfo=pytz.UTC)
        ...
        self._metadata_array = torch.from_numpy(
            self.metadata[self._metadata_fields].astype(int).to_numpy()
        ).long()[~seq_mask]
    ...
    def eval(self, y_pred, y_true, metadata, prediction_fn=None):
        ...
        region_results = metric.compute_group_wise(
            y_pred, y_true, region_grouper.metadata_to_group(metadata), region_grouper.n_groups
        )
        ...
        all_results_str += (
            f"Worst-group {metric.name}: {all_results[f'{metric.name}_worst_region']:.3f}\n"
        )
