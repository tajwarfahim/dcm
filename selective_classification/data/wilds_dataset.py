...
class WILDSDataset:
    ...
    def _add_coarse_domain_metadata(self):
        ...
        self._metadata_array = torch.cat([self._metadata_array, from_source_domain], dim=1)
    ...
    def source_domain_splits(self):
        ...
        """
        return getattr(
            self, "_source_domain_splits", WILDSDataset.DEFAULT_SOURCE_DOMAIN_SPLITS
        )
    ...
    def dataset_exists_locally(self, data_dir, version_file):
        ...
        return os.path.exists(data_dir) and (
            ...
            or (len(os.listdir(data_dir)) > 0 and download_url is None)
        )
    ...
    def download_dataset(self, data_dir, download_flag):
        ...
        print(
            f"You can also download the dataset manually at https://wilds.stanford.edu/downloads."
        )
...
class WILDSSubset(WILDSDataset):
    ...
    def __getitem__(self, idx):
        x, y, _, metadata = self.dataset[self.indices[idx]]
