...
def plot_figure(
    train_stats, val_stats, legends, x_label, y_label, title, figure_path, show_image
):
...
def plot_loss(model_name, train_loss_history, val_loss_history, directory, show_image=False):
    ...
    plot_figure(
        train_stats=train_loss_history,
...
def plot_accuracy(
    ...
    plot_figure(
        train_stats=train_accuracy_history,
...
def plot_id_and_ood_scores(
    id_scores, ood_scores, score_type, id_dataset, ood_dataset, figure_path
):
    fig = plt.figure(figsize=(10, 5))
    sns.kdeplot(x=id_scores, shade=True, color="b", label="ID")
    sns.kdeplot(x=ood_scores, shade=True, color="r", label="OOD")
    plt.xlabel(f"Maximum Scores ({score_type})", fontsize="large")
    plt.ylabel("Frequency", fontsize="large")
    plt.title(f"Confidence Scores (ID={id_dataset}, OOD={ood_dataset})", fontsize="x-large")
    plt.legend()
