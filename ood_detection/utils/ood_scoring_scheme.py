...
def calculate_pairwise_elementwise_absolute_distance_matrix(
    matrix_A, matrix_B, siamese_network_version
):
    ...
    if siamese_network_version == 1:
        ...
        raise ValueError(
            "Siamese network version:",
            siamese_network_version,
            " is not supported, it has to be either 1, 2 or 3",
        )
...
def recover_siamese_network_weights(siamese_network, siamese_network_version):
    ...
    if siamese_network_version in [1, 2]:
        combined_weight = np.append(
            np.reshape(a=siamese_net_weight, newshape=(-1,)),
            np.reshape(a=siamese_net_bias, newshape=(-1,)),
            axis=0,
        )
...
def pairwise_comparison_scores(
    siamese_network,
    embeddings_of_test_dataset,
    embeddings_of_ID_dataset,
    num_divides_of_ID_dataset,
    siamese_network_version,
    temp,
):
    # assertions to check if inputs are valid
    assert isinstance(embeddings_of_test_dataset, np.ndarray) and isinstance(
        embeddings_of_ID_dataset, np.ndarray
    )
    ...
    if siamese_network_version in [1, 2]:
        embeddings_of_test_dataset = append_constant_value_column_to_matrix(
            matrix=embeddings_of_test_dataset, value=1
        )
        embeddings_of_ID_dataset = append_constant_value_column_to_matrix(
            matrix=embeddings_of_ID_dataset, value=0
        )

    # preparing the weights
    if siamese_network_version in [1, 2, 3]:
        siamese_net_weight = recover_siamese_network_weights(
    ...
    for i in range(num_divides_of_ID_dataset):
        partial_ID_dataset_embedding = embeddings_of_ID_dataset[
            i * each_division_len : (i + 1) * each_division_len, :
        ]
        partial_distance_matrix = calculate_pairwise_elementwise_absolute_distance_matrix(
            matrix_A=embeddings_of_test_dataset,
            matrix_B=partial_ID_dataset_embedding,
            siamese_network_version=siamese_network_version,
        )

        if siamese_network_version in [1, 2]:
            partial_scores = np.einsum(
                "ijk,k->ij", partial_distance_matrix, siamese_net_weight
            )
            scores.append(np.transpose(partial_scores))

        elif siamese_network_version == 3:
            partial_M_distance_matrix = np.einsum(
                "ijk,kl->ijl", partial_distance_matrix, siamese_net_weight
            )
            partial_scores = np.einsum(
                "ijk,ijk->ij", partial_distance_matrix, partial_M_distance_matrix
            )
            scores.append(np.transpose(partial_scores))
...
def first_reduction(scores_3D, name_accumulator, axis):
    ...
    if name_accumulator == "max - min":
        ...
        first_accumulator = assign_accumulator_function_from_name(
            function_name=name_accumulator
        )
...
def second_reduction(scores_2D, name_accumulator):
    ...
    if name_accumulator == "max - min":
        ...
        second_accumulator = assign_accumulator_function_from_name(
            function_name=name_accumulator
        )
...
def process_comparison_scores(
    ...
    scores_3D = np.reshape(
        a=scores, newshape=(num_test_examples, num_ID_classes, num_samples_per_ID_class)
    )

    if order == "across_examples_first":
        scores_after_first_reduction = first_reduction(
            scores_3D=scores_3D,
            name_accumulator=name_accumulator_across_examples_in_a_class,
            axis=2,
        )
        scores_after_second_reduction = second_reduction(
            scores_2D=scores_after_first_reduction,
            name_accumulator=name_accumulator_across_classes,
        )
...
def get_classification_accuracy(
    ...
    scores_3D = np.reshape(
        a=id_test_scores,
        newshape=(num_test_examples, num_ID_classes, num_samples_per_ID_class),
    )
    accumulated_scores = first_reduction(
        scores_3D=scores_3D,
        name_accumulator=name_accumulator_across_examples_in_a_class,
        axis=2,
    )
...
def get_all_embeddings(
    ...
    for key in dataloader_map:
        embeddings_map[key] = precalculate_embeddings(
            siamese_network, dataloader_map[key], False
        )
...
class OODScoreCalculator:
    def __init__(
        self, siamese_network, id_train_dataloader, id_test_dataloader, ood_dataloader
    ):
    ...
    def prepare_embeddings(self, save_directory, device_type):
        self.embeddings_map = get_all_embeddings(
    ...
    def run_pairwise_comparison(
        self, num_divides_of_ID_dataset=50, siamese_network_version=2, temp=0.1
    ):
        assert self.embeddings_map is not None

        self.id_pairwise_comparison_scores = pairwise_comparison_scores(
            siamese_network=self.siamese_network,
        ...
        self.ood_pairwise_comparison_scores = pairwise_comparison_scores(
            siamese_network=self.siamese_network,
    ...
    def calculate_id_test_classification_accuracy(
        self, name_accumulator_across_examples_in_a_class, sample_size_per_class
    ):
        assert self.id_pairwise_comparison_scores is not None

        classification_accuracy = get_classification_accuracy(
            id_test_scores=self.id_pairwise_comparison_scores,
            id_test_dataloader=self.id_test_dataloader,
    ...
    def get_id_and_ood_score(
        ...
        assert name_accumulator_across_examples_in_a_class in [
            "average",
            "min",
            "max",
            "max - min",
        ...
        id_scores, id_scores_after_class_reduction = process_comparison_scores(
            scores=self.id_pairwise_comparison_scores,
            name_accumulator_across_examples_in_a_class=name_accumulator_across_examples_in_a_class,
        ...
        ood_scores, ood_scores_after_class_reduction = process_comparison_scores(
            scores=self.ood_pairwise_comparison_scores,
            name_accumulator_across_examples_in_a_class=name_accumulator_across_examples_in_a_class,
            name_accumulator_across_classes=name_accumulator_across_classes,
            sample_size_per_class=sample_size_per_class,
            order=order,
