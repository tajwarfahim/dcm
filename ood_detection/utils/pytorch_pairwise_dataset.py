    ...
    if dataset_name == "CIFAR10":
        ...
        if train:
            ...
            extra_dataset = datasets.SVHN(
                root=path, split="extra", download=True, transform=transform
            )
            dataset = torch.utils.data.ConcatDataset(datasets=[train_dataset, extra_dataset])
        ...
        dataset = TinyImageNet(root_dir=path, split=split, transform=transform)
...
def get_custom_data_transform(dataset_name, augment, mean, std):
    if dataset_name == "SVHN":
        ...
        elif dataset_name in ["CelebA", "STL10"]:
            transform = transforms.Compose(
                [
                    transforms.Resize(size=(32, 32)),
                    transforms.ToTensor(),
...
def load_dataset_with_custom_data_transform(dataset_name, train, path, transform):
    if dataset_name == "CIFAR10":
        ...
        dataset = CIFAR100Coarse(root=path, train=train, download=True, transform=transform)
    ...
    elif dataset_name in ["TinyImageNet_Resize", "LSUN_Resize"]:
...
def load_dataset(
    ...
    if id_dataset_name == "TinyImageNet":
        mean = [0.485, 0.456, 0.406]
        std = [0.229, 0.224, 0.225]

        custom_transform = get_custom_data_transform(
            dataset_name=dataset_name, augment=augment, mean=mean, std=std
        )

    elif id_dataset_name in ["CIFAR10", "CIFAR100"]:
    ...
    elif id_dataset_name == "SVHN":
        mean = [0.0, 0.0, 0.0]
        std = [1.0, 1.0, 1.0]
        custom_transform = transforms.ToTensor()

    else:
        id_train_dataset = load_dataset_with_basic_transform(
            dataset_name=id_dataset_name, train=True, path=id_dataset_path
        )
...
def generate_per_class_sample(dataset_name, dataset_path, save_path, train, sample_size):
    dataset = load_dataset(
        dataset_name=dataset_name, train=train, path=dataset_path, download=True
    )
...
def generate_validation_dataset(
    dataset_name, dataset_path, save_path, half_validation_dataset_size
):
    dataset = load_dataset(
        dataset_name=dataset_name, train=False, path=dataset_path, download=True
    )
    mapping = create_label_to_index_mapping(dataset)
    labels = [key for key in mapping.keys()]
    labels.sort()
    ...
    np.savetxt(
        save_path, np.array(validation_dataset, dtype=np.int32), delimiter=",", fmt="%d"
    )
...
class SampledDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return self.dataset[remapped_index]
...
class PreSavedPerClassSampledDataset(Dataset):
    def __init__(self, dataset, sample_indices_path):
        ...
        self.class_to_sample_indices_mapping = load_per_class_samples(
            save_path=sample_indices_path
        )
        assert is_valid_class_to_sample_mapping(self.class_to_sample_indices_mapping)

        self.class_labels = [
            class_label for class_label in self.class_to_sample_indices_mapping
        ]
        assert len(self.class_labels) > 0
        self.class_labels.sort()
        self.sample_size_per_class = len(
            self.class_to_sample_indices_mapping[self.class_labels[0]]
        )
...
class SelectiveClassDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, start_label, end_label):
        self.dataset = dataset
