...
class OODMetricsCalculator:
    def __init__(self, id_scores, ood_scores, id_dataset_name, ood_dataset_name):
        ...
        self.combined_scores, self.y = prepare_scores_and_labels(
            id_scores=self.id_scores, ood_scores=self.ood_scores
        )

    def get_roc_auc_score(self, name_accumulator_across_classes):
        scores = (
            self.combined_scores
            * self.score_multiplier_map_AU[name_accumulator_across_classes]
        )
    ...
    def get_fpr(self, name_accumulator_across_classes, recall_level):
        ...
        fpr = fpr_and_fdr_at_recall(
            y_true=self.y, y_score=scores, recall_level=recall_level, pos_label=None
        )
        fpr = round(fpr * 100, 1)
