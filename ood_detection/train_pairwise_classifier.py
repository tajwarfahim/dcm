...
def parse_arguments():
    ...
    ap.add_argument(
        ...
        "--partial_dataset_filename",
        type=str,
        default="dataset_1/partial_dataset_labels.txt",
    )
...
def create_dataloaders(args):
    ...
    if args["base_rate"] < 1.0:
        train_dataset = RandomlySampledDataset(
            dataset=train_dataset, base_rate=args["base_rate"], choose_randomly=False
        )
    ...
    if args["use_partial_dataset"] == 1:
        ...
        validation_dataset = SelectiveClassDataset(
            dataset=validation_dataset, labels_path=labels_path
        )

    check_dataset(dataset=train_dataset)
    check_dataset(dataset=validation_dataset)

    pairwise_train_dataset = PairwiseDatasetRandom(
        dataset=train_dataset, epoch_length=args["train_epoch_size"]
    )
    pairwise_validation_dataset = PairwiseDatasetRandom(
        dataset=validation_dataset, epoch_length=int(args["train_epoch_size"] / 5)
    )

    # random pairs are formed during training, so shuffling is not necessary
    train_dataloader = torch.utils.data.DataLoader(
        dataset=pairwise_train_dataset,
    ...
    validation_dataloader = torch.utils.data.DataLoader(
        dataset=pairwise_validation_dataset,
...
def create_optimizer_and_scheduler(args, model, len_train_dataloader):
    ...
    pairwise_optimizer = torch.optim.SGD(
        model_parameters,
        lr=args["lr"],
    ...
    if args["use_default_scheduler"] == 1:
        pairwise_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            pairwise_optimizer, T_max=args["num_epochs"]
        )
    else:
        ...
        pairwise_scheduler = torch.optim.lr_scheduler.LambdaLR(
            pairwise_optimizer,
...
def run_model_training(args):
    ...
    model_trainer = PairwiseModelTrainer(
        model=model,
