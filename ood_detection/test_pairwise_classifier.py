...
def parse_arguments():
    ...
    ap.add_argument(
        "-use_first_samples", "--use_first_samples", type=int, default=0, choices=[0, 1]
    )
    ...
    ap.add_argument(
        "-embedding_save_directory",
        "--embedding_save_directory",
        type=str,
        default=None,
    )
    ap.add_argument(
        "-threshold_for_accuracy", "--threshold_for_accuracy", type=float, default=None
    )
    ap.add_argument(
        "-num_divides_of_ID_dataset",
        "--num_divides_of_ID_dataset",
        type=int,
        default=50,
    )
    ap.add_argument(
        "-outlier_exposure", "--outlier_exposure", type=int, default=1, choices=[0, 1]
    )
    ...
    ap.add_argument(
        "-name_accumulator_across_classes",
        "--name_accumulator_across_classes",
        type=str,
        default="average",
        choices=["max", "average", "min", "max - min", "all"],
    )
    ap.add_argument(
        "-name_accumulator_across_examples_in_a_class",
        "--name_accumulator_across_examples_in_a_class",
        type=str,
        default="average",
        choices=["max", "average", "min", "max - min", "all"],
    ...
    ap.add_argument(
        ...
        "--accumulation_order",
        type=str,
        default="across_examples_first",
        choices=["across_examples_first", "across_classes_first"],
    )
...
def create_resnet_model(args):
    dataset_name = args["id_dataset_name"]
    kwargs = {
        "model_name": args["shared_model_name"],
        "num_classes": args["num_classes"],
    }
...
def prepare_model(args):
    model = create_siamese_network_wrapper(
        dataset_name=args["id_dataset_name"],
        shared_model_name=args["shared_model_name"],
        load_to_gpu=False,
    ...
    if args["siamese_network_version"] == [1, 2, 3, 4]:
        model.load_state_dict(
            torch.load(f=args["presaved_model_path"], map_location=torch.device("cpu"))
        )
        ...
        if args["fine_tune"] > 0:
            ...
            model.shared_model.load_state_dict(
                torch.load(f=args["presaved_model_path"], map_location=torch.device("cpu"))
            )
...
def prepare_experiment_plot_directory(
    args, name_accumulator_across_examples_in_a_class, name_accumulator_across_classes
):
    model_timestamp_str = args["presaved_model_path"].split("/")[-1]
    dataset_names = "id_" + args["id_dataset_name"] + "_ood_" + args["ood_dataset_name"]
    function_names = (
        "accumulator_across_examples="
        + name_accumulator_across_examples_in_a_class
        + "_accumulator_across_classes="
        + name_accumulator_across_classes
    )

    plot_directory = os.path.join(
        args["plot_save_path"], model_timestamp_str, dataset_names, function_names
    )
...
def prepare_metrics_for_plotting(
    ...
    for sample_size in sample_sizes:
        value_of_metric = performance_record[sample_size][
            name_accumulator_across_examples_in_a_class
        ][name_accumulator_across_classes][metric_name]
...
def plot_experiment_results(
    ...
    performance_record,
    metric_names,
    names_accumulator_across_examples_in_a_class,
    names_accumulator_across_classes,
):
    print("Metric names: ", metric_names)

    for (
        name_accumulator_across_examples_in_a_class
    ) in names_accumulator_across_examples_in_a_class:
        for name_accumulator_across_classes in names_accumulator_across_classes:
            plot_directory = prepare_experiment_plot_directory(
                args=args,
            ...
            for metric_name in metric_names:
                ...
                metrics, sample_sizes = prepare_metrics_for_plotting(
...
def get_id_and_ood_test_dataloaders(args):
    ...
    if args["id_selective_class"] == 1:
        id_test_dataset = SelectiveClassDataset(
            dataset=id_test_dataset, labels_path=args["id_labels_path"]
        )

    if args["ood_selective_class"] == 1:
        ood_test_dataset = SelectiveClassDataset(
            dataset=ood_test_dataset, labels_path=args["ood_labels_path"]
        )
    ...
    id_test_dataloader = torch.utils.data.DataLoader(
...
def get_id_train_dataloaders(args, sample_size_lst):
    ...
    for sample_size in sample_size_lst:
        ...
        if args["use_first_samples"] > 0:
            dataset = RandomlySampledDataset(
            ...
            dataset = PreSavedPerClassSampledDataset(
                dataset=id_train_dataset,
                sample_indices_path=sample_indices_filenames[sample_size],
            )

        else:
            selective_class_dataset = SelectiveClassDataset(
                dataset=id_train_dataset, labels_path=args["id_labels_path"]
            )

            dataset = PreSavedPerClassSampledDataset(
                dataset=selective_class_dataset,
                sample_indices_path=sample_indices_filenames[sample_size],
            )
...
def prepare_embeddings_for_the_experiment(ood_score_calculator, args, device_type):
    ...
    ood_score_calculator.prepare_embeddings(
        save_directory=args["embedding_save_directory"], device_type=device_type
    )
    end_time = time.time()
...
def prepare_pairwise_comparison_scores_for_the_experiment(ood_score_calculator, args):
    print_message(
        message="Starting pairwise comparison between ID train and test sets (ID test and OOD test)..."
    )

    start_time = time.time()

    ood_score_calculator.run_pairwise_comparison(
        siamese_network_version=args["siamese_network_version"],
...
def print_metrics(args, perfomance_metrics):
    print()
    print(
        "Accumulator across ID classes: ",
        perfomance_metrics["name_accumulator_across_classes"],
    )
    print(
        "Accumulator across examples in a class: ",
        perfomance_metrics["name_accumulator_across_examples_in_a_class"],
    )
    ...
    print(
        "ID test classification accuracy: ",
        perfomance_metrics["classification_accuracy"],
    )
    print(
        "FPR at recall recall level ",
        args["recall_level"],
        ": ",
        perfomance_metrics["fpr"],
    )
...
def plot_auroc_and_aupr(
    ...
    plot_subdirectory = os.path.join(
        plot_directory, "sample_size_per_ID_class=" + str(sample_size)
    )
    ...
    metrics_calculator.draw_model_roc_curve(
        name_accumulator_across_classes=name_accumulator_across_classes,
        save_path=auroc_file_path,
    )
    metrics_calculator.draw_model_pr_curve(
        name_accumulator_across_classes=name_accumulator_across_classes,
        save_path=aupr_file_path,
    )
...
def run_a_single_experiment(
    args,
    ood_score_calculator,
    name_accumulator_across_examples_in_a_class,
    name_accumulator_across_classes,
    sample_size,
):
    classification_accuracy = ood_score_calculator.calculate_id_test_classification_accuracy(
        name_accumulator_across_examples_in_a_class=name_accumulator_across_examples_in_a_class,
        sample_size_per_class=sample_size,
    )

    id_scores, ood_scores = ood_score_calculator.get_id_and_ood_score(
        name_accumulator_across_examples_in_a_class=name_accumulator_across_examples_in_a_class,
        name_accumulator_across_classes=name_accumulator_across_classes,
        sample_size_per_class=sample_size,
    ...
    metrics_calculator = OODMetricsCalculator(
        id_scores=id_scores,
    ...
    auroc = metrics_calculator.get_roc_auc_score(
        name_accumulator_across_classes=name_accumulator_across_classes
    )
    aupr_in = metrics_calculator.get_pr_auc_score(
        name_accumulator_across_classes=name_accumulator_across_classes, positive="in"
    )
    aupr_out = metrics_calculator.get_pr_auc_score(
        name_accumulator_across_classes=name_accumulator_across_classes, positive="out"
    )
    fpr = metrics_calculator.get_fpr(
        name_accumulator_across_classes=name_accumulator_across_classes,
    ...
    perfomance_metrics = {
        ...
        "name_accumulator_across_examples_in_a_class": name_accumulator_across_examples_in_a_class,
        "name_accumulator_across_classes": name_accumulator_across_classes,
        "id_average": id_average,
        "ood_average": ood_average,
...
def run_experiment_over_different_comparison_functions(
    args,
    sample_size,
    model,
    id_train_dataloader,
    ...
    ood_score_calculator = OODScoreCalculator(
    ...
    prepare_embeddings_for_the_experiment(
        ood_score_calculator=ood_score_calculator, args=args, device_type=device_type
    )
    prepare_pairwise_comparison_scores_for_the_experiment(
        ood_score_calculator=ood_score_calculator, args=args
    )
...
def run_experiment_over_different_sample_sizes(
    args,
    model,
    id_train_dataloaders,
    id_test_dataloader,
    ...
    for sample_size in sample_size_lst:
        ...
        performance_record_per_sample_size[
            sample_size
        ] = run_experiment_over_different_comparison_functions(
            args=args,
            sample_size=sample_size,
            model=model,
...
def run_experiments(
    ...
    performance_record = run_experiment_over_different_sample_sizes(
        args=args,
        model=model,
    ...
    if args["threshold_for_accuracy"] is not None:
        ...
        metric_names = [
            ...
            "AUROC",
            "AUPR",
            "Accuracy_with_tpr_n_threshold",
            "Accuracy_with_fpr_n_threshold",
        ]
...
def run_script(args):
    ...
    if args["name_accumulator_across_examples_in_a_class"] == "all":
        ...
        names_accumulator_across_examples_in_a_class = [
            args["name_accumulator_across_examples_in_a_class"]
        ]
    ...
    performance_record, metric_names = run_experiments(
        args=args,
        sample_size_lst=sample_size_lst,
        names_accumulator_across_examples_in_a_class=names_accumulator_across_examples_in_a_class,
        names_accumulator_across_classes=names_accumulator_across_classes,
